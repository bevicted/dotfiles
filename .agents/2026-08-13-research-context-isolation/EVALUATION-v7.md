# Research context isolation evaluation v7

**Status:** replacement design, not sealed or executed.
**Supersedes:** v6 execution eligibility. The inherited v2 source is committed blob `500840b6ec51b589134ea122d792249ea6372d01` at `3b018aa608dbf8bf437b3956decfbfd5b1da8db5`.
...
**Inherited v6 source:** `.agents/2026-08-13-research-context-isolation/EVALUATION-v6.md`, committed blob `2036a6b9f398c8f8ab147570b762c7a1a930e581` at v6 design-seal commit `3cf98010fc1c0555f706012179dcf6dbc7a8c81d`.
**Frozen implementation baseline:** `249b9fe290e0bfa984d6b72406fbdde8271f475e`.

This replacement retains the v6 protocol and corrections. It resolves only v6 downstream's unloaded production boundary: both downstream arms now load the production subagent index before capture. V6 also exposed the `thinkingSignature` value-taint continuation defect, fixed at this baseline. It does not alter cases, packets, prompts, caps, budgets, command order, no-retry rule, scoring, or strict context/provider leakage gates.

## 1. Inherited protocol and versioned audit path

The complete v7 protocol starts with the exact pinned v6 source above. Replace every literal `audit/v6` path in that source with `audit/v7`, then apply only the authoritative changes in this file: Section 2 replaces revision eligibility and its verification; Section 3 replaces its runtime-preflight records; Sections 4 and 5 retain v6's dependency-free child helper, child-extension preflight, and ten-route fixture correction at the v7 path; Section 6 replaces only the downstream command and its provider-tools assertion; and Section 7 updates the disposition. No other inherited text changes. This construction retains every v6, v5, and v4 correction, including retained preflight records, the ten-route fixture manifest, child-session custom-entry checks, and all cases and gates.

If this file conflicts with v2, v3, v4, v5, or the pinned v6 source, this file controls only on revision eligibility, the retained eligibility artifact, audit-directory creation, fixture route count, preflight retention, child-extension isolation, the top-level `PI_SUBAGENT_DEPTH` requirement, or the downstream production-boundary load and provider-tools assertion. All other v2 requirements, including every prompt, packet, budget, command order, no-retry rule, scoring rule, and context/provider gate, remain literal and mandatory.

The raw artifact directory is `.agents/2026-08-13-research-context-isolation/audit/v7/`, ignored by the existing repository `.gitignore` rule. Before the first runtime-preflight command, create only its empty `preflight/` directory and the retained revision-eligibility Python artifact specified in Section 2. `run-manifest.json` is fixed at `audit/v7/preflight/run-manifest.json` and is permitted there during runtime preflight. Do not create a fixture, trace, session, envelope, score, citation audit, or usage calculation until the runtime preflight passes. Create the rest of `audit/v7/` only when proceeding to the fixture preflight and cells. Raw artifacts remain lossless and unredacted as v2 requires.
...
1. `D` must have `I` as its first parent. Its diff from `I` must contain exactly `.agents/2026-08-13-research-context-isolation/EVALUATION-v7.md`. It must retain the unchanged committed v2, v4, v5, and v6 blobs named above and all prior v3, v4, v5, and v6 design and attempt records already present at `I`. This is the required standalone design-seal commit and must exist before any scored invocation.
2. `E` may equal `D` or descend from `D`. Between `D` and `E`, only `.agents/2026-08-13-research-context-isolation/EVALUATION-RESULTS.md` may change before cells launch. This permits a tracked v7 preflight-only result without changing the design or implementation.
3. At `E`, every tracked implementation path must have the same blob as `I`. The only tracked paths permitted to differ from `I` are `EVALUATION-v7.md` and `EVALUATION-RESULTS.md` in this task directory. Verify this mechanically with a path/blob comparison, not commit messages.
4. Run all inherited commands from the repository root at `E`, with a clean worktree except ignored `audit/v7/` artifacts created by this procedure. Record `I`, `D`, and `E` in `audit/v7/preflight/run-manifest.json` and the results report.

Before the fixture server starts, create the exact executable Python artifact below at `audit/v7/preflight/revision-eligibility.py`, then retain that file unchanged. Its SHA-256 is `5db11d564aad70c23ace821c9f8b8b27ce15a33ad88807b7daaf6ce12bd2b08b`. The artifact is the eligibility program: execute this file directly, never supply its program through standard input. Retain the artifact itself and the raw records for both its SHA-256 check and execution under the names `revision-eligibility-artifact-sha256` and `revision-eligibility`.
...
I = "249b9fe290e0bfa984d6b72406fbdde8271f475e"
...
V6_BLOB = "2036a6b9f398c8f8ab147570b762c7a1a930e581"
RESULTS_PATH = ".agents/2026-08-13-research-context-isolation/EVALUATION-RESULTS.md"
V7_PATH = ".agents/2026-08-13-research-context-isolation/EVALUATION-v7.md"
...
require(git("diff", "--name-only", "--no-renames", I, D).splitlines() == [V7_PATH], "I..D must change only EVALUATION-v7.md")
...
require(blob(E, V6_PATH) == V6_BLOB, "pinned v6 source blob changed")
require(blob(E, V7_PATH) == blob(D, V7_PATH), "v7 design changed after D")
...
require(differing_paths <= {V7_PATH, RESULTS_PATH}, {"unexpected_path_blob_differences": sorted(differing_paths - {V7_PATH, RESULTS_PATH})})
...
```sh
cat > .agents/2026-08-13-research-context-isolation/audit/v7/preflight/revision-eligibility.py <<'PY'
#!/usr/bin/env python3
import re
import subprocess
import sys
...
print({"I": I, "D": D, "E": E, "pathBlobDifferences": sorted(differing_paths)})
PY
chmod 700 .agents/2026-08-13-research-context-isolation/audit/v7/preflight/revision-eligibility.py
...
expected = "5db11d564aad70c23ace821c9f8b8b27ce15a33ad88807b7daaf6ce12bd2b08b"
actual = hashlib.sha256(path.read_bytes()).hexdigest()
if actual != expected:
    raise SystemExit({"expected": expected, "actual": actual})
print(actual)
' .agents/2026-08-13-research-context-isolation/audit/v7/preflight/revision-eligibility.py
```
...
env -u PI_SUBAGENT_DEPTH git rev-parse HEAD | tee .agents/2026-08-13-research-context-isolation/audit/v7/preflight/revision-discovery-design-seal.stdout
DESIGN_SEAL="$(<.agents/2026-08-13-research-context-isolation/audit/v7/preflight/revision-discovery-design-seal.stdout)"

# revision-discovery-execution-revision
env -u PI_SUBAGENT_DEPTH git rev-parse HEAD | tee .agents/2026-08-13-research-context-isolation/audit/v7/preflight/revision-discovery-execution-revision.stdout
EXECUTION_REVISION="$(<.agents/2026-08-13-research-context-isolation/audit/v7/preflight/revision-discovery-execution-revision.stdout)"
export DESIGN_SEAL EXECUTION_REVISION

# revision-eligibility
env -u PI_SUBAGENT_DEPTH ./.agents/2026-08-13-research-context-isolation/audit/v7/preflight/revision-eligibility.py "$DESIGN_SEAL" "$EXECUTION_REVISION"
```
...
The retained v3 cell attempt and v4, v5, and v6 attempts remain invalid attempts under their own designs; none is a v7 cell and none can be retried. V7 begins with zero launched cells. Once a v7 cell launches, retain and score no retry exactly as v2 requires. Any later implementation change requires another separately fixed implementation revision and a new versioned replacement design with a new baseline and seal; it cannot be folded into v7.

## 3. Retained runtime preflight and top-level environment

For every command below, retain its exact expanded argv, including the `env -u PI_SUBAGENT_DEPTH` wrapper, and selected environment-key names in `audit/v7/preflight/<name>.argv.json`, unmodified stdout in `<name>.stdout`, unmodified stderr in `<name>.stderr`, and exit status, start/end UTC, cwd, and signal in `<name>.status.json`. Retain empty stdout or stderr files. `run-manifest.json` may summarize these records but never substitutes for them. Store credential-bearing environment variables only as `present: true|false` and never retain their values.
...
1. The `revision-eligibility` command in Section 2. It defines and verifies `I`, `D`, and `E`, performs the path/blob comparison, and checks the v2 and pinned-v4/v5/v6 source blobs.
...
5. `env -u PI_SUBAGENT_DEPTH git -c core.excludesFile=/dev/null check-ignore -v --no-index .agents/2026-08-13-research-context-isolation/audit/v7/probe`. It must identify the repository `.gitignore` rule.
...
# Downstream, both direct and isolated arms
env -u PI_SUBAGENT_DEPTH sh -c 'cat "$1" | env -u PI_SUBAGENT_DEPTH PI_OFFLINE=1 pi --mode json -p --no-context-files --no-skills --no-prompt-templates --model openai-codex/gpt-5.6-sol:high --session "$2" --no-tools --no-extensions --extension .pi/agent/extensions/subagent/index.ts --extension .agents/2026-08-13-research-context-isolation/audit/v7/capture-extension.ts' sh "$PROMPT" "$SESSION"
```

For every direct and isolated downstream `before_provider_request` capture, assert that the provider payload exposes no tools: `payload.tools` must be absent or an empty array. A nonempty tool definition, any other advertised tool, or a missing provider capture invalidates that downstream phase and cell.

## 7. Final disposition

This file is intentionally unsealed until committed under Section 2. The prior v3 design and invalid cell attempt, and the v4, v5, and v6 designs and invalid attempts, remain unchanged and are not v7 work. Task 06 remains unchecked unless a future fully valid v7 run supplies all inherited required raw artifacts and measurements, the retained preflight records above, the retained and verified revision-eligibility artifact, and all strict v2 gates.
