...
- Changes: Preserved the invalid v4 and v5 preflight-only attempts. V6 retained all 22 fixed-order launched phases and raw preflight, fixture, session, capture, child-copy, budget, citation, and scoring artifacts. The retained-artifact review corrected only v6 derived analysis, results, and these notes.
- Decisions or deviations: Task 06 remains unchecked. V4 remains invalid because its top-level Git preflight records retained `PI_SUBAGENT_DEPTH=1`, and its required child-extension argv/path command failed. V5 correctly unset that variable, but used abbreviated revision IDs and an unretained stdin eligibility program. V6 preflight and fixture checks passed, but every isolated downstream `context` leaked raw markers despite zero markers in its paired provider payload: C1 A/B/C 507 each; C2 Operations 78 and Legal 82; C3 76; C4 74; C5 A/B 88. C5 isolated continuation failed before a new Research result with `No tool call found for function call output`, so it made no C5-C fetch, did not create a new budget ledger, and has no continuation envelope or provenance. The completed C1-C4 and C5-fresh Research envelopes are bounded and match their raw Research content; the 117-byte failure text is an assistant/provider response, not a Research envelope. No attempt was adapted or retried.
- Verification results: Retained v6 raw records show the exact executable revision-eligibility artifact and full D/E discovery, child argv and hash checks, unset top-level depth marker, and ordered ten-route fixture manifest. All 22 Pi processes exited 0; C5 continuation's provider failure is retained with `willRetry: false`. The fixture access log records the expected 19 successful GETs before that failed continuation. Re-analysis from retained raw artifacts makes every isolated cell invalid and non-score-eligible; local Research-phase observations remain distinct from final cell validity. Retained final verification records show 57/57 focused subagent/Research tests passed, 89/91 web tests passed with two optional live tests skipped, and offline extension loading passed.
- Remaining risks or follow-ups: Preserve v4, v5, v6 raw artifacts and frozen designs unchanged. Task 06 fails all comparable-valid-cell and parent-isolation gates; downstream quality is not measurable. Do not start Task 07.

### V7 retained-artifact correction

- Changes: Recomputed only ignored v7 derived analysis/audits from retained raw artifacts and appended the v7 disposition to `EVALUATION-RESULTS.md`. No model cell, fixture, prompt, frozen design, implementation file, or raw attempt was changed or rerun.
- Decisions or deviations: Task 06 remains unchecked. C1-C4 isolated Research and C5 fresh place a bounded, marker-free report in the `context` capture, but the immediately following provider payload emits a `function_call_output` with only `call_id`, no `output` or `content`, injects the synthetic Research-isolation-failure text, and includes an unnamed `webfetch` schema. This is a provider-boundary failure, so all isolated cells are invalid and non-score-eligible. The five fresh isolated 14/14 reports are qualitative observations only; all isolated downstream outputs are absent.
- Verification results: The v7 access log has 19 successful expected GETs through isolated C5 fresh and no isolated C5-C GET. C5 continuation made no Research call and ended with `Codex error: The 'None' model is not supported when using Codex with a ChatGPT account.` and `willRetry: false`; it has no C5-C fetch, ledger, resumed report, child copy, or hash. Its extension, custom-type, budget, provenance, and same-child checks are unmeasured, not passed. C1-C4 and C5-fresh child copies still parse, match their hashes, and contain only allowed Research custom types.
- Final checks: Focused subagent/Research tests passed 58/58; web tests passed 89/91 with two optional live tests skipped; offline subagent extension loading and `git diff --check` passed.
- Remaining risks or follow-ups: Task 06 fails all four gates. The observed direct and isolated usage totals are not an efficiency comparison because isolated totals include failed provider requests. Do not retry retained cells or start Task 07.
