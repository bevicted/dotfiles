...
import { RESEARCH_CONTEXT_ENTRY, isResearchContextTelemetry, type ResearchContextTelemetry } from "./research-context-audit.ts";
import { RESEARCH_AGENT_NAME, RESEARCH_MODEL, RESEARCH_TOOLS, type NormalizedResearchInput } from "./research.ts";
...
const RESEARCH_ID_PATTERN = /^r_[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/;
...
function trustedTools(value: unknown): value is string[] {
	return Array.isArray(value) && value.every((tool) => typeof tool === "string" && RESEARCH_TOOLS.includes(tool as typeof RESEARCH_TOOLS[number])) && new Set(value).size === value.length;
}

function isMetadata(value: unknown): value is ResearchSessionMetadata {
	if (!value || typeof value !== "object" || Array.isArray(value)) return false;
	const entry = value as Record<string, unknown>;
	return entry.version === RESEARCH_SESSION_VERSION &&
		typeof entry.researchId === "string" && RESEARCH_ID_PATTERN.test(entry.researchId) &&
		typeof entry.parentSessionId === "string" && entry.parentSessionId.length > 0 &&
		typeof entry.cwd === "string" && entry.cwd.length > 0 &&
		entry.agent === RESEARCH_AGENT_NAME && entry.model === RESEARCH_MODEL &&
		trustedTools(entry.tools) && typeof entry.createdAt === "string";
}

function isMapping(value: unknown): value is ResearchSessionMapping {
	return isMetadata(value) && typeof (value as Record<string, unknown>).updatedAt === "string";
}
...
function writeBootstrap(child: SessionManager): void {
	...
	if (entries.some((entry) => entry === null)) throw new Error("Research session could not be initialized.");
 ...
 */
export function isTrustedResearchChildSession(session: ReadonlySessionManager, childSessionId: unknown): boolean {
	if (typeof childSessionId !== "string" || childSessionId !== session.getSessionId()) return false;
	const header = session.getHeader();
	const first = session.getEntries()[0];
	const childFile = session.getSessionFile();
	if (!header || header.id !== childSessionId || header.cwd !== session.getCwd() || !childFile) return false;
	if (first?.type !== "custom" || first.customType !== RESEARCH_CHILD_ENTRY || !isMetadata(first.data)) return false;
	const metadata = first.data;
	const parentFile = (header as unknown as { parentSession?: unknown }).parentSession;
	if (typeof parentFile !== "string" || !parentFile) return false;
	...
	if (!parentHeader || parentHeader.id !== metadata.parentSessionId || parent.getSessionFile() !== parentFile ||
		path.resolve(header.parentSession) !== path.resolve(parentFile)) return false;
	...
	try {
		childPaths = fs.readdirSync(parent.getSessionDir())
			.filter((file) => file.endsWith(".jsonl"))
			.map((file) => path.join(parent.getSessionDir(), file))
			.filter((file) => {
				try {
					return SessionManager.open(file, parent.getSessionDir()).getSessionId() === childSessionId;
				} catch {
					return false;
				}
			});
	...
	if (childPaths.length !== 1 || path.resolve(childPaths[0]) !== path.resolve(childFile)) return false;

	const mapping = [...parent.getEntries()].reverse().find((entry) => entry.type === "custom" && entry.customType === RESEARCH_MAPPING_ENTRY && isMapping(entry.data) && entry.data.researchId === metadata.researchId);
	if (!mapping || !isMapping(mapping.data)) return false;
	const data = mapping.data as ResearchSessionMapping & { childSessionId?: unknown };
	return data.childSessionId === childSessionId && data.researchId === metadata.researchId &&
		data.parentSessionId === metadata.parentSessionId && data.cwd === metadata.cwd &&
		data.agent === metadata.agent && data.model === metadata.model && data.createdAt === metadata.createdAt &&
...
export class ResearchSessionStore {
	...
	private readonly inMemory = new WeakMap<object, Map<string, ResearchSessionTarget>>();
	private readonly active = new Map<string, { lockFile: string; token: string }>();
	...
	create(parent: ResearchSessionParent, cwd: string, effectiveTools: readonly string[]): ResearchSessionTarget {
		...
		if (!parentIsPersisted(parent)) {
			const targets = this.inMemory.get(parent as object) ?? new Map<string, ResearchSessionTarget>();
	...
	resume(parent: ResearchSessionParent, cwd: string, researchId: string, effectiveTools: readonly string[]): ResearchSessionTarget {
		validateResearchId(researchId);
		const canonical = canonicalCwd(cwd);
		if (!parentIsPersisted(parent)) {
			const target = this.inMemory.get(parent as object)?.get(researchId);
			if (!target || target.parentSessionId !== parent.getSessionId() || target.cwd !== canonical || !sameStrings(target.effectiveTools, effectiveTools)) {
				throw new Error(unavailable);
			}
			return { ...target, effectiveTools: [...target.effectiveTools], resumed: true };
		}

		const mapping = this.findMapping(parent, researchId);
		if (!mapping || mapping.parentSessionId !== parent.getSessionId() || mapping.cwd !== canonical || !sameStrings(mapping.tools, effectiveTools)) {
			throw new Error(unavailable);
		}
		...
		try {
			matches = (fs.readdirSync(parent.getSessionDir()) as string[])
				...
				.filter((file) => {
					try {
						return SessionManager.open(file).getSessionId() === mapping.childSessionId;
					} catch {
						return false;
					}
				})
		...
		const metadata = firstEntry?.type === "custom" && firstEntry.customType === RESEARCH_CHILD_ENTRY && isMetadata(firstEntry.data)
			? firstEntry.data
			: undefined;
		if (!header || header.cwd !== canonical || header.parentSession !== parent.getSessionFile() || !metadata ||
			metadata.researchId !== researchId || metadata.parentSessionId !== parent.getSessionId() || metadata.cwd !== canonical ||
			metadata.agent !== RESEARCH_AGENT_NAME || metadata.model !== RESEARCH_MODEL || !sameStrings(metadata.tools, effectiveTools) ||
	...
	lock(target: ResearchSessionTarget): void {
		...
		if (this.active.has(key)) throw new Error("Research continuation is already running for this Research ID.");
		if (!target.sessionFile) throw new Error(unavailable);
		const lockFile = `${target.sessionFile}.research-lock`;
		const token = randomUUID();
		try {
			fs.writeFileSync(lockFile, JSON.stringify({ token }), { encoding: "utf8", mode: 0o600, flag: "wx" });
			this.active.set(key, { lockFile, token });
		} catch {
			throw new Error("Research continuation is already running for this Research ID.");
		}
	}
	...
	release(target: ResearchSessionTarget): void {
		...
		try {
			const lock = JSON.parse(fs.readFileSync(held.lockFile, "utf8")) as { token?: unknown };
	...
	maskingTelemetry(target: ResearchSessionTarget): ResearchContextTelemetry[] {
		try {
			...
			return child.getEntries()
				.filter((entry) => entry.type === "custom" && entry.customType === RESEARCH_CONTEXT_ENTRY && isResearchContextTelemetry(entry.data))
	...
	private findMapping(parent: ResearchSessionParent, researchId: string): (ResearchSessionMapping & { childSessionId: string }) | undefined {
		for (const entry of [...parent.getEntries()].reverse()) {
			if (entry.type !== "custom" || entry.customType !== RESEARCH_MAPPING_ENTRY || !isMapping(entry.data)) continue;
			const data = entry.data as ResearchSessionMapping & { childSessionId?: unknown };
			if (data.researchId !== researchId || typeof data.childSessionId !== "string" || !data.childSessionId) continue;
...
export function researchSessionError(error: unknown): string {
	...
	return message === unavailable || message.startsWith("Research continuation is") || message.startsWith("Research researchId") ? message : unavailable;
}

export function researchSessionMetadataForDetails(target: ResearchSessionTarget): Omit<ResearchSessionTarget, "sessionFile"> {
	const { sessionFile: _sessionFile, ...safe } = target;
	return { ...safe, effectiveTools: [...safe.effectiveTools] };
}

export function researchSessionHandoff(input: NormalizedResearchInput): Pick<NormalizedResearchInput, "task" | "context" | "files" | "webResearch" | "effort"> {
	return { task: input.task, context: input.context, files: [...input.files], webResearch: input.webResearch, effort: input.effort };
}
