...
- Give each implementation task one cohesive independently verifiable outcome and a narrow, complete path through every affected layer. Acceptance criteria may cover the data, behavior, interface, tests, configuration, migration, and documentation required by that outcome. Split multiple outcomes when each can leave the repository valid.
...
- Scope each task for one fresh agent session. Split it when discovery, implementation, and verification contain independently verifiable outcomes, or when one agent could not plausibly complete the task coherently in one session. Do not use an arbitrary token, file-count, or acceptance-count threshold.
