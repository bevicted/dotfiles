...
Zig development is funded via Zig Software Foundation,
    ...
    Roadmap to 1.0. If you need donation receipts or are
...
Perhaps most notably, this release debuts I/O as an Interface,
    but don't sleep on the Language Changes or enhancements to
    the Compiler, Build System, Linker,
    Fuzzer, and Toolchain which are also included in this release.
...
## Target Support §
      ...
      Support Table and Additional Platforms sections cover
...
- 
        Stack tracing support has been significantly improved across the board;
        almost all major targets now provide stack traces on crashes.
      
      
- 
        Various Standard Library bugs that mainly affected weakly-ordered
...
- 
        Various Standard Library and Compiler bugs preventing Zig
...
- The Compiler can generate machine code for these targets without relying on LLVM.
...
- The Standard Library cross-platform abstractions account for these targets.
...
- The Compiler can generate machine code for these targets via LLVM.
      
- The Linker can produce object files, libraries, and executables for these targets.
      
- These targets are not considered experimental by LLVM.
...
- The Compiler can generate assembly source code for these targets via LLVM.
...
In the future, C Translation will be handled via the
    Build System rather than the @cImport
...
Alternately, you can add
    the official translate-c package
...
Zig 0.16.0 implements long-accepted proposal #10710 to remove the
...
Note that due to Forbid Enum and Packed Types with Implicit Backing Types in Extern Contexts,
...
If this has introduced a compile error in your code, resolve it by adding an explicit tag
    type or backing type. (See Allow Explicit Backing Integers on Packed Unions for a
...
/home/ci/.cache/act/fae00571f8d8c29e/hostexecutor/src/download/0.16.0/release-notes/extern_implicit_backing_type.zig:5:1: error: unable to export type 'extern_implicit_backing_type.Enum'
export var some_enum: Enum = .a;
^~~~~~
/home/ci/.cache/act/fae00571f8d8c29e/hostexecutor/src/download/0.16.0/release-notes/extern_implicit_backing_type.zig:1:14: note: integer tag type of enum is inferred
const Enum = enum { a, b, c, d };
             ^~~~~~~~~~~~~~~~~~~
/home/ci/.cache/act/fae00571f8d8c29e/hostexecutor/src/download/0.16.0/release-notes/extern_implicit_backing_type.zig:1:14: note: consider explicitly specifying the integer tag type
/home/ci/.cache/act/fae00571f8d8c29e/hostexecutor/src/download/0.16.0/release-notes/extern_implicit_backing_type.zig:1:14: note: enum declared here
/home/ci/.cache/act/fae00571f8d8c29e/hostexecutor/src/download/0.16.0/release-notes/extern_implicit_backing_type.zig:6:1: error: unable to export type 'extern_implicit_backing_type.PackedStruct'
export var some_packed_struct: PackedStruct = .{ .a = 1, .b = 2 };
^~~~~~
/home/ci/.cache/act/fae00571f8d8c29e/hostexecutor/src/download/0.16.0/release-notes/extern_implicit_backing_type.zig:6:1: note: inferred backing integer of packed struct has unspecified signedness
/home/ci/.cache/act/fae00571f8d8c29e/hostexecutor/src/download/0.16.0/release-notes/extern_implicit_backing_type.zig:2:29: note: struct declared here
const PackedStruct = packed struct { a: u4, b: u4 };
                     ~~~~~~~^~~~~~~~~~~~~~~~~~~~~~~
/home/ci/.cache/act/fae00571f8d8c29e/hostexecutor/src/download/0.16.0/release-notes/extern_implicit_backing_type.zig:7:1: error: unable to export type 'extern_implicit_backing_type.PackedUnion'
export var some_packed_union: PackedUnion = .{ .a = 123 };
^~~~~~
/home/ci/.cache/act/fae00571f8d8c29e/hostexecutor/src/download/0.16.0/release-notes/extern_implicit_backing_type.zig:7:1: note: inferred backing integer of packed union has unspecified signedness
/home/ci/.cache/act/fae00571f8d8c29e/hostexecutor/src/download/0.16.0/release-notes/extern_implicit_backing_type.zig:3:28: note: union declared here
const PackedUnion = packed union { a: u8, b: i8 };
                    ~~~~~~~^~~~~~~~~~~~~~~~~~~~~~
...
A problem we noticed since introducing I/O as an Interface is
...
- Io.Threaded - based on threads. With this
      implementation, I/O operations are straightforward. For example, File System
      ...
      including Cancelation. This is the implementation currently chosen by
      "Juicy Main".
...
- Io.Uring - although it was not the
        ...
        Networking, error handling, test coverage, and minimal task
        stack allocations.
...
- Future - task-level abstraction based on functions. Allows introducing
      operational independence (asynchrony) among any set of function calls.
    
- Group - efficiently manages many independent tasks. Supports awaiting and
      canceling all tasks in the group together.
...
- Select - executes tasks together, providing a mechanism to wait until one or more tasks complete. Similar to Batch but operates at the higher level task abstraction layer rather than lower level Operation abstraction layer.
    
- Batch - lower level abstraction based on introducing independence among any set of operations.
...
- On Windows, this all happens without ws2_32.dll dependency.
    
    
init: std.process.Init is thanks to "Juicy Main".
...
Future, Group, and Batch APIs all support
...
Eventually most of the File System and Networking
...
Meanwhile Future is the equivalent but at a function
...
So, generally, if you're trying to write optimal, reusable software,
    ...
    Future APIs if that would essentially require you to reinvent
    futures. Or you can start with Future APIs and then optimize by
...
These APIs also integrate properly with Cancelation.
...
Lock-free and thread-safe plays better with std.Io integration and
    libc integration. By avoiding locks, we avoid needing Sync Primitives and thereby
...
same thing is planned for heap.DebugAllocator
...
Additionally, inline callers are now resolved from debug info when
    ...
    ambiguous, all candidate callers are printed.
    Support for resolving
    inline traces from DWARF is planned.
...
Zig 0.16.0 reworks many standard library APIs related to debug information, and in particular
    ...
    memory addresses) without introducing potential crashes in cases where frame pointers are
...
This fixes a handful of bugs, makes Cancelation and
    Batch work properly for networking operations, and avoids the
...
Notably the Batch API and Cancelation have full Windows support
...
Users who wish to target older versions of Windows such as XP, or for
    ...
    as kernel32 are encouraged to collaborate on a third-party I/O implementation
    that eschews NtDll.
...
The "environment" (a set of key-value string mappings inherited by child
    ...
    without any kind of lock. Additionally, the Zig standard library had a major footgun:
...
Now, environment variables are available only in the application's main function.
    ...
    *const process.Environ.Map parameter. An instance of this
    environment variable map can be obtained conveniently from "Juicy Main".
...
Or simply get them from "Juicy Main" via std.Process.Init.preopens.
...
- Added heap.MemoryPoolUnmanaged, heap.MemoryPoolAlignedUnmanaged, heap.MemoryPoolExtraUnmanaged (#23234)
...
Removing std.builtin.subsystem is a breaking change but it is unlikely many
    ...
    If
    your code absolutely needs to know the subsystem there are ways to determine it at runtime.
...
This API was a bit too opinionated for the Zig standard library.
    ...
    Users may consider third party package known-folders
    as an alternative.
...
The motivation for this change is to make it easier to tinker. Go ahead
    ...
    Run baobab on your dependency tree.
...
Zig no longer observes ZIG_BTRFS_WORKAROUND environment variable. The
    bug has been fixed in upstream Linux a long time ago by now (#17095).
...
Zig's implementation of translate-c is now based on
    arocc and translate-c instead of libclang. Goodbye and good riddance to 5,940 lines of our remaining C++ code in
    the compiler source tree, with 3,763 remaining.
    
The implementation is compiled lazily from source the first time @cImport is encountered. In the future, Zig will drop the @cImport language builtin, but for
...
Matthew also looked into changing the representation of tagged union and error union types in debug
    information to use variant types,
    ...
    support, but 
    it's only enabled when the language is marked as Rust). He may revisit this in the future if
...
Compared to the x86 Backend, the LLVM backend is passing
...
- Extremely poor code quality in degenerate cases
...
Zig 0.16.0 significantly reworks
    ...
    of compiler bugs, in particular related to Incremental Compilation.
...
/home/ci/.cache/act/fae00571f8d8c29e/hostexecutor/src/download/0.16.0/release-notes/struct_uses_own_alignment.zig:2:28: error: type 'struct_uses_own_alignment.S' depends on itself for alignment query here
    foo: [*]align(@alignOf(@This())) u8,
                           ^~~~~~~
...
error: dependency loop with length 3
    /home/ci/.cache/act/fae00571f8d8c29e/hostexecutor/src/download/0.16.0/release-notes/complex_dependency_loop.zig:6:29: note: default field value of 'complex_dependency_loop.S' uses value of declaration 'complex_dependency_loop.default_val' here
    const S = struct { x: u32 = default_val };
                                ^~~~~~~~~~~
    /home/ci/.cache/act/fae00571f8d8c29e/hostexecutor/src/download/0.16.0/release-notes/complex_dependency_loop.zig:7:21: note: value of declaration 'complex_dependency_loop.default_val' uses value of declaration 'complex_dependency_loop.other_val' here
    const default_val = other_val;
                        ^~~~~~~~~
    /home/ci/.cache/act/fae00571f8d8c29e/hostexecutor/src/download/0.16.0/release-notes/complex_dependency_loop.zig:8:19: note: value of declaration 'complex_dependency_loop.other_val' uses default field values of 'complex_dependency_loop.S' here
    const other_val = @typeInfo(S).@"struct".fields.len;
                      ^~~~~~~~~~~~
...
If you are struggling to resolve a dependency loop, consider joining a Zig
    community to get help from fellow Zig users!
...
- Incremental updates have been made significantly faster by avoiding "over-analysis" (where
        ...
        thanks to Reworked Type Resolution making the compiler's internal dependency graph
...
- When using a self-hosted backend targeting ELF, the New ELF Linker is now enabled
        by default, which is faster and has much more stable support for incremental compilation.
        This linker is not yet feature-complete—see New ELF Linker for details.
...
- The LLVM Backend now supports incremental compilation. This does not speed up the
...
Incremental compilation still has known bugs, including some
    miscompilations, and therefore remains
...
Because incremental compilation is now usable with both the self-hosted ELF linker
    and the LLVM Backend, opting in is usually as simple as running
...
Future release cycles will continue to focus on
    incremental compilation, with more bug fixes, improved testing
    infrastructure, performance enhancements, and better Linker
    support.
...
- Generates better constant memcpy code (#25353).
    
    
Compared to the LLVM Backend, this backend passes more behavior
...
Still a work-in-progress. Progress was paused during this release cycle
    due to the I/O as an Interface churn. Currently crashes when
    running the behavior tests. Progress is expected to pick up as the
    Standard Library churn subsides.
...
Compared to the LLVM Backend, Zig's WebAssembly backend is passing
...
Eliminates a dependency on LLVM with regards to the set of MinGW-w64 .def
...
Performance data point [source]:
    building the Zig Compiler, then making a single-line change to a function, and then another:
...
This has been reported and
    fixed upstream, however at time
...
When targeting musl statically, many functions are now provided by zig libc rather
...
However, many functions are now provided by zig libc rather
...
In spite of this, the number of WASI libc C source files distributed with Zig increased
    ...
    source files are shared with musl.
...
Zig's libc implementation gained many new functions, leading to the corresponding C source
    files being deleted from musl and MinGW-w64. In this release,
...
Notably this includes many math functions, as well as malloc and friends.
    Special thanks to Szabolcs Nagy for libc-test.
...
9 bugs were fixed: GitHub Codeberg
...
The 0.17.0 release cycle will be short, aiming mainly to upgrade to LLVM 22 and finish
    separating the make process (build runner) from the configure process (build.zig).
...
- Completing the aarch64 Backend, making it the default backend for debug mode.
      
- Enhancing Linker implementations, eliminating dependency on
      LLD and supporting Incremental Compilation.
      
- Enhancing the integrated Fuzzer to be competitive with AFL and other state-of-the-art fuzzers.
      
- Transitioning from a library dependency on LLVM to a process dependency on Clang.
...
## Thank You Sponsors! §
    ...
    using our preferred donation platform:
