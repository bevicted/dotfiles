...
3. Set the ticket-set ID and output directory. When the source plan is `.agents/<ticket-set-id>/PLAN.md`, reuse its `<ticket-set-id>` and directory for the ticket set. Otherwise choose an ID in the form `<YYYY-MM-DD>-<short-kebab-slug>`. Reuse an existing ID when updating the same ticket set. If it belongs to different work, add a short distinguishing suffix. Never overwrite unrelated tickets.
4. Create only `.agents/<ticket-set-id>/TICKETS.md` in the repository root. For a source plan in `.agents/<ticket-set-id>/PLAN.md`, create `TICKETS.md` beside it. Preserve the source artifact in place. Link to a durable source from `TICKETS.md`; when the source is conversation context, summarize all context needed to make the file self-contained.
